        ...
        <div class="container">
            <div class="row align-items-center">
                ...
                <div class="col-lg-5 ms-auto">
                    <div class="image-box">
                        <img class="w-100 rounded lazy" src="{{ $packages->getFirstMediaUrl() }}" alt="Package Image">
                    </div>
                </div>

                <div class="col-lg-5">
                    <div class="hero-content py-5">
                        <h3 class="fs-19 fw-medium pb-3">{{ $packages->title }}</h3>
                        <p class="section-desc pb-4">{!! $packages->description !!}</p>
                        ...
                        <div class="price-box mb-4 mt-3 p-3"
                            ...
                            <span class="price fw-bold"
                                style="color: #e74c3c; font-size: 24px;">৳{{ number_format($packages->price) }}</span>
                        </div>
                        <form action="{{ route('cart.add') }}" method="POST">
                            @csrf
                            <input type="hidden" name="package_id" value="{{ $packages->id }}">
                            <button type="submit" class="btn theme-btn">Buy Now</button>
                        </form>
                    </div>
                </div>
            </div>
