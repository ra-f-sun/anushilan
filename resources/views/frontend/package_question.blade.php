    ...
    <section class="get-started-area pt-80px pb-50px pattern-bg bg-gray">
        <div class="container">
            <div class="text-center">
                <h2 class="section-title">প্যাকেজ তালিকা</h2>
                    ...
                    <div class="col-lg-4 responsive-column-half">
                        <div class="card card-item hover-y text-center">
                            <div class="card-body">
                                ...
                                <h5 class="card-title pt-4 pb-2">{!! $value->title !!}</h5>
                                <p class="card-text">
                                    {{ \Illuminate\Support\Str::words(strip_tags($value->description), 20, '...') }}
                                </p>
                                <p class="card-amount">Price: ৳{{ $value->price }}</p>
                                @if (in_array($value->id, $userPackageIds))
                                    <a href="{{ route('show.package.category', ['id' => $value->id]) }}"
                                        class="btn btn-success">View My Package</a>
                                @else
                                    <a href="{{ route('packageDetails', ['id' => $value->id]) }}"
